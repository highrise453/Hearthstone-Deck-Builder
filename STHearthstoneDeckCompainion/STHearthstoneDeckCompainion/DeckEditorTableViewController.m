//
//  DeckEditorTableViewController.m
...
@interface DeckEditorTableViewController ()


@end

@implementation DeckEditorTableViewController

@synthesize cardArray;
@synthesize deckEditorSearchBar;
...
- (void) tableView: (UITableView *) tableView didSelectRowAtIndexPath: (NSIndexPath *) indexPath{
    if (deckArray.count < 30) {
        id card;
        if (filteredCardArray >0) {
            card =[filteredCardArray objectAtIndex:indexPath.row];
        }
        if (filteredCardArray == 0) {
            card = [cardArray objectAtIndex:indexPath.row];
        }
        NSString *rarity = [card valueForKey:@"rarity"];
        int count = 0;
        if ([rarity isEqual:@"Legendary"]){
            count = 1;
        }
        ...
        if( count < 2){
           [deckArray addObject:card];
            ...
            NSLog(@"Maximum amount of this card is in the deck");
 
    }
    else
        [self performSegueWithIdentifier:@"deckViewSegue" sender:indexPath];
        
}
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if ([[segue identifier] isEqualToString:@"deckViewSegue"]) {
        
        DeckViewTableViewController *vc = [segue destinationViewController];
    
        [vc setDeck:[self deckArray]];
    }
}
